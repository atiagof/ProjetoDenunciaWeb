SSPApp.controller('ctrlDescartar', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL) {

    $scope.listaDescartados = null;

    $scope.gridDescartados = {
        columnDefs: [
            {
            ...
            { name: 'denuncia', displayName: 'Protocolo', minWidth: 60 },
            ...
            { name: 'data_atendimento', displayName: 'Data de atendimento', type: 'date', cellFilter: "date: 'dd/MM/yyyy'" },
            { name: 'data_descarte', displayName: 'Data de descarte', type: 'date', cellFilter: "date: 'dd/MM/yyyy'" },
            { name: 'justificativa', displayName: 'Motivo' },
        ],
    ...
    $scope.carregarLista = function () {
        ...
        if (user != null) {
            var serv = baseURL + '/api/Atendimento/listarDescartes';
            var met = 'GET';
            ...
            .then(
                function successCallback(response) {
                    $scope.listaDescartados = response.data;
                    $scope.gridDescartados.data = response.data;
                }
            ...
            ajaxError('Não foi possível carregar a lista de descartes.');
        }
    };
    ...
    $scope.atualizarDenuncia = function (ano, num) {
        ...
        if (user != null) {
            var serv = baseURL + '/api/Atendimento/atualizarDescarte';
            var met = 'POST';
            var head = { 'Content-Type': 'application/json' };
            ...
            .then(
                function successCallback(response) {
                    $cookies.put('anoDescarte', ano, { path: '/' });
                    $cookies.put('numDescarte', num, { path: '/' });
                    $window.location.href = baseURL + '/Atendimento/DescarteDenuncia';
                }
                , function errorCallback(response) {
                    ajaxError(response.data);
                });
    ...
    $scope.abrirDenuncia = function (ano, numero) {
        var checkAno = $cookies.get('anoDescarte');
        var checkNum = $cookies.get('numDescarte');
