SSPApp.controller('ctrlValidaDenuncia', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL) {
    ...
    $scope.lim_retorno = 1000;
    $scope.orgResposta = '';

    // Carrega as informações de acordo com os dados passados
    $scope.carregar = function () {
        var ano = $cookies.get('anoValidar');
        var numero = $cookies.get('numValidar');
    ...
    // Função para checar o limite do preenchimento da justificativa do retorno
    $scope.checarRetorno = function () {
        if ($scope.retorno.length > $scope.lim_retorno)
            $scope.retorno = $scope.retorno.slice(0, $scope.lim_retorno);
    }
    ...
    $scope.verificarDenuncia = function () {
        ...
        if (user != null) {
            if ($scope.denuncia.usuario_validacao.toUpperCase() != user.NOME.toUpperCase())
                return true;
            else
                return false;
        }
    ...
    $scope.verificarRetorno = function () {
        if ($scope.verificarDenuncia()) {
            $('#descRetorno').modal('hide');
            $('#checkRet').modal({});
        } else {
            $scope.retornarDenuncia();
        }
    }

    $scope.verificarValidacao = function () {
        if ($scope.verificarDenuncia()) {
            $('#descValidar').modal('hide');
            $('#checkVal').modal({});
        } else {
            $scope.validarDenuncia();
        }
    }

    // 
    $scope.retornarDenuncia = function () {
        var user = $cookies.getObject('retorno');

        if (user != null)
        {
            if ($scope.retorno == null || $scope.retorno.trim() == '') {
                ajaxError('Para que a denúncia seja retornada, é necessário preencher o motivo do retorno!');
            }
            else {
                var serv = baseURL + '/api/Atendimento/retornarDenuncia';
                var met = 'POST';
                var head = { 'Content-Type': 'application/json' };
                var data = { ano: $scope.ano, numero: $scope.numero, idAtend: user.ID_USUARIO, just: $scope.retorno, civil: user.FLAG_CIVIL, militar: user.FLAG_MILITAR };
            ...
            ajaxError('Não foi possível retornar a denúncia.')
        }
    }

    //
    $scope.validarDenuncia = function () {
        var user = $cookies.getObject('retorno');

        if (user != null)
        {
            if ($scope.resposta == null || $scope.resposta.trim() == '') {
                ajaxError('Para que a denúncia seja validada, é necessário preencher a resposta!');
            }
            else {
                var serv = baseURL + '/api/Atendimento/validarDenuncia';
                var met = 'POST';
                var head = { 'Content-Type': 'application/json' };
                var data = { ano: $scope.ano, numero: $scope.numero, idAtend: user.ID_USUARIO, resposta: $scope.resposta, civil: user.FLAG_CIVIL, militar: user.FLAG_MILITAR };
            ...
            ajaxError('Não foi possível validar a denúncia.')
        }
    }

    // 
    $scope.copiaResposta = function () {
        $scope.resposta = $scope.denuncia.resposta;
    }

    // Retorna o usuário para a lista de análise
    $scope.retornar = function () {
        $cookies.remove('anoValidar', { path: '/' });
        $cookies.remove('numValidar', { path: '/' });
        $window.location.href = baseURL + '/Atendimento/Validar';
    }
    ...
    $scope.exportar = function () {
        if ($scope.denuncia != null) {
            ...
            // Descarte
            conteudo.push({ text: 'Apuração', style: 'den_title' });
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Usuário que esta validando a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_validacao }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Policial que respondeu a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_resposta }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Unidade policial: ', bold: true }, { text: $scope.denuncia.orgao }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'A apuração da denúncia foi finalizada com resultado positivo? ', bold: true }, { text: $scope.denuncia.flag_resposta }] });
            conteudo.push({ text: ' ', style: 'den_space2' });

            if ($scope.denuncia.flag_resposta == "Sim") {
                conteudo.push({ text: [{ text: 'A apuração da denúncia resultou no registro de um boletim de ocorrência? ', bold: true }, { text: $scope.denuncia.flag_registroBO }] });
                conteudo.push({ text: ' ', style: 'den_space2' });

                if ($scope.denuncia.flag_registroBO == "Sim") {
                    conteudo.push({ text: [{ text: 'Data de providência: ', bold: true }, { text: $scope.denuncia.data_providencia }] });
                    conteudo.push({ text: ' ', style: 'den_space2' });
                    conteudo.push({ text: [{ text: 'Tipo de boletim: ', bold: true }, { text: $scope.denuncia.tipo_boletim }] });
                    conteudo.push({ text: ' ', style: 'den_space2' });
                    conteudo.push({ text: [{ text: 'Delegacia de elaboração: ', bold: true }, { text: $scope.denuncia.delegacia_elaboracao }] });
                    conteudo.push({ text: ' ', style: 'den_space2' });
                    conteudo.push({ text: [{ text: 'Número de boletim: ', bold: true }, { text: $scope.denuncia.numero_boletim }] });
                    conteudo.push({ text: ' ', style: 'den_space2' });
                }
            }
            ...
            pdfMake.createPdf($scope.exportDen).download('Validação - ' + protocolo + '.pdf');
        }
    }
