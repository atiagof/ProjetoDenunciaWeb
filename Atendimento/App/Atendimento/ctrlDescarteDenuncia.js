SSPApp.controller('ctrlDescarteDenuncia', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL) {

    $scope.denuncia = null;
    $scope.vazio = null;
    $scope.reativo = '';
    $scope.lim_reativo = 1000;

    // Carrega as informações de acordo com os dados passados
    $scope.carregar = function () {
        var ano = $cookies.get('anoDescarte');
        var numero = $cookies.get('numDescarte');
    ...
    $scope.checarReativo = function () {
        if ($scope.reativo.length > $scope.lim_reativo)
            $scope.reativo = $scope.reativo.slice(0, $scope.lim_reativo);
    }
    ...
    $scope.verificarDenuncia = function () {
        ...
        if (user != null) {
            if ($scope.denuncia.usuario_descarte.toUpperCase() != user.NOME.toUpperCase())
                return true;
            else
                return false;
        }
    ...
    $scope.verificarReativacao = function () {
        if ($scope.verificarDenuncia()) {
            $('#reatModal').modal('hide');
            $('#checkReat').modal({});
        } else {
            $scope.reativarDenuncia();
        }
    }

    $scope.verificarConfirmacao = function () {
        if ($scope.verificarDenuncia()) {
            $('#descModal').modal('hide');
            $('#checkConf').modal({});
        } else {
            $scope.confirmarDescarte();
        }
    }

    // Realiza o encaminhamento para o orgão mencionado
    $scope.reativarDenuncia = function () {
        var user = $cookies.getObject('retorno');

        if (user != null) {
            if ($scope.reativo == null || $scope.reativo.trim() == '') {
                ajaxError('Selecione a unidade policial que receberá a denúncia!');
            }
            else {
                var serv = baseURL + '/api/Atendimento/reativarDenuncia';
                var met = 'POST';
                var head = { 'Content-Type': 'application/json' };
                var data = { ano: $scope.ano, numero: $scope.numero, idAtend: user.ID_USUARIO, just: $scope.reativo, civil: user.FLAG_CIVIL, militar: user.FLAG_MILITAR };
            ...
            ajaxError('Não foi possível reativar a denúncia.')
        }
    };

    // Marca a denúncia como descartada para entrar na lista de validação
    $scope.confirmarDescarte = function () {
        var user = $cookies.getObject('retorno');

        if (user != null) {
            var serv = baseURL + '/api/Atendimento/confirmarDescarte';
            var met = 'POST';
            var head = { 'Content-Type': 'application/json' };
            var data = { ano: $scope.ano, numero: $scope.numero, idAtend: user.ID_USUARIO, civil: user.FLAG_CIVIL, militar: user.FLAG_MILITAR };
            ...
            ajaxError('Não foi possível confirmar o descarte da denúncia.')
        }
    }

    // Retorna o usuário para a lista de análise
    $scope.retornar = function () {
        $cookies.remove('anoDescarte', { path: '/' });
        $cookies.remove('numDescarte', { path: '/' });
        $window.location.href = baseURL + '/Atendimento/Descartar';
    }
    ...
    $scope.exportar = function () {
        if ($scope.denuncia != null) {
            ...
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Atendente que está validando o descarte da denúncia: ', bold: true }, { text: $scope.denuncia.usuario_descarte }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Atendente que descartou a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_atendimento }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Motivo do descarte: ', bold: true }, { text: $scope.denuncia.just_descarte }], style: 'just_text' });
            conteudo.push({ text: ' ', style: 'den_space2' });
            ...
            pdfMake.createPdf($scope.exportDen).download('Descartado - ' + protocolo + '.pdf');
        }
    }
