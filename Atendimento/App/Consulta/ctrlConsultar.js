var ctrlConsulta;

SSPApp.controller('ctrlConsulta', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', '$filter', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL, $filter) {
    angular.extend(this, $controller('BaseListController', { $scope: $scope }));
    ctrlConsulta = $scope;

    $scope.ConsultaViewModel = {
        DataEncaminhamentoFinal: ""
    };

    $scope.Expand = '&$expand=tipo_crime,orgao,situacao';
    $scope.gridOptions.enableFiltering = false;

    $scope.DataMaxima = new Date();

    // Seleciona a primeira opção do combo Expirar
    $scope.expirar = "";
    $scope.finalizada = "1";

    // Função de validação da regra Expirar
    $scope.Expirar = function (row) {

        if (!angular.isDefined(row.entity.data_encaminhamento) ||
            row.entity.data_encaminhamento == null ||
            row.entity.data_encaminhamento == "") {
            return "";
        }
        var data = new Date(row.entity.data_encaminhamento);
        //alert(data.getYear());

        //return row.entity.data_encaminhamento;
        var d = new Date(data.getYear(), data.getMoth(), data.getDate());
        alert(data.getYear());
        return row.entity.data_encaminhamento;
    };

    // Função para formatar a data
    $scope.formatJSONDate = function (jsonDate) {

        if (jsonDate != null) {
            var dataFormatada = $filter('date')(jsonDate, 'dd/MM/yyyy');
            return dataFormatada;
        }
    }

    // Configuração do Grid para exportação do pdf gerado
    $scope.gridExport.exporterPdfDefaultStyle = { fontSize: 9 };
    //$scope.gridExport.exporterPdfTableStyle = { margin: [5, 0, 0, 5] };
    $scope.gridExport.exporterPdfTableHeaderStyle = { fontSize: 10, bold: true, italics: true, color: 'steelblue' };
    $scope.gridExport.exporterPdfOrientation = 'landscape';
    $scope.gridExport.exporterPdfPageSize = 'A4';
    $scope.gridExport.exporterPdfMaxGridWidth = 670;

    // Botão Exportar Excel
    $scope.exportarExcel = function () {

        // Trata as data dentro do objeto antes da geração do pdf
        for (var i = 0; i < $scope.gridExportApi.grid.rows.length; i++) {
            $scope.gridExportApi.grid.rows[i].entity.data_inclusao = $scope.formatJSONDate($scope.gridExportApi.grid.rows[i].entity.data_inclusao);
            $scope.gridExportApi.grid.rows[i].entity.data_encaminhamento = $scope.formatJSONDate($scope.gridExportApi.grid.rows[i].entity.data_encaminhamento);
            $scope.gridExportApi.grid.rows[i].entity.data_expira = $scope.formatJSONDate($scope.gridExportApi.grid.rows[i].entity.data_expira);
        }

        var myElement = angular.element(document.querySelectorAll(".custom-csv-link-location"));
        $scope.gridExportApi.exporter.csvExport("all", "visible", myElement);

    };

    // Configuração do Grid para exportação do csv gerado
    $scope.gridExport.exporterCsvFilename = 'ConsultaDenuncia.csv';
    $scope.gridExport.exporterCsvColumnSeparator= ';';
    $scope.gridExport.exporterOlderExcelCompatibility = true;
    $scope.gridExport.exporterCsvLinkElement =  angular.element(document.querySelectorAll(".custom-csv-link-location"));
    $scope.gridExport.enableColumnResizing = true;

    // Configuração das colunas exibidas do grid gridExport *Exportação
    $scope.gridExport.columnDefs = [
        {
            name: 'numero_denuncia',
            cellTemplate: '<div style="text-align: center; padding: 5px 0 5px 0;">D{{row.entity.numero_denuncia}}{{row.entity.ano_denuncia }}</div>',
            displayName: 'Protocolo',
            minWidth: 60, Width: 60
        },
        { name: 'data_inclusao', displayName: 'Data de cadastro', type: 'date', cellFilter: "date: 'dd/MM/yyyy'" },
        { name: 'cpf_denunciante', displayName: 'CPF do denunciante' },
        { name: 'situacao.DESCRICAO_SITUACAO', displayName: 'Situação' },
        { name: 'tipo_crime.DESCRICAO_TIPO_CRIME', displayName: 'Tipo de crime' },
        

        { name: 'data_encaminhamento', displayName: 'Data de encaminhamento', type: 'date', cellFilter: "date: 'dd/MM/yyyy'" },

        { name: 'orgao.NOME_ORGAO', displayName: 'Unidade Policial' },
        { name: 'data_expira', displayName: 'A expirar em', type: 'date', cellFilter: "date: 'dd/MM/yyyy'", enableSorting: false }
    ];

    // Configuração das colunas exibidas no grid da tela
    $scope.gridOptions.columnDefs = [
        {
            name: "Visualizar", displayName: '', enableCellEdit: false, minWidth: 40, maxWidth: 60, enableFiltering: false,
            cellTemplate: '<div style="text-align: center; padding: 5px 0 5px 0;"><button type="button" class="btn fa fa-search bg-yellow" ng-click="grid.appScope.abrirDenuncia(row.entity.ano_denuncia,row.entity.numero_denuncia)"></button></div>'
        },

        {
            name: 'numero_denuncia',
            cellTemplate: '<div style="text-align: center; padding: 5px 0 5px 0;">D{{row.entity.numero_denuncia}}{{row.entity.ano_denuncia }}</div>',
            displayName: 'Protocolo',
            minWidth: 60, Width: 60
        },
        { name: 'data_inclusao', displayName: 'Data de cadastro', type: 'date', cellFilter: "date: 'dd/MM/yyyy HH:mm:ss'" },
        { name: 'cpf_denunciante', displayName: 'CPF do denunciante' },
        { name: 'situacao.DESCRICAO_SITUACAO', displayName: 'Situação' },
        { name: 'tipo_crime.DESCRICAO_TIPO_CRIME', displayName: 'Tipo de crime' },        

        { name: 'data_encaminhamento', displayName: 'Data de encaminhamento', type: 'date', cellFilter: "date: 'dd/MM/yyyy'" },

        { name: 'orgao.NOME_ORGAO', displayName: 'Unidade Policial' },
        { name: 'data_expira', displayName: 'A expirar em', type: 'date', cellFilter: "date: 'dd/MM/yyyy'", enableSorting: false }
    ];

    $scope.carregarLista = function () {
        var user = $cookies.getObject('retorno');

        if (user != null) {
            var serv = baseURL + '/odata/DenunciaConsultaModel';
            var met = 'GET';
            var head = { 'Content-Type': 'application/json' };
            $scope.listaApuracao = null;

            $http({ method: met, url: serv, headers: head })
            .then(
                function successCallback(response) {
                    $scope.listaApuracao = response.data;
                    $scope.gridApuracao.data = response.data;
                }
                , function errorCallback(response) {
                    ajaxError(response.data);
                });
        }
        else {
            ajaxError('Não foi possível carregar a lista de denúncias.');
        }
    };

    $scope.abrirDenuncia = function (ano, numero) {
        $scope.carregar(numero + "" + ano);
    };

    $scope.listTipoOrgao = [];

    // Protocolo - Carrega as informações de acordo com os dados passados
    $scope.carregar = function (numeroDenuncia) {
        var user = $cookies.getObject('retorno');

        var numero = "";

        if (numeroDenuncia != null && numeroDenuncia.length > 0) {
            numero = numeroDenuncia;
        }
        else {
            numero = $scope.ConsultaViewModel.NumeroProtocolo;
        }

        var ano = numero.substr(numero.length - 4, 4);

        numero = numero.replace("D", "");
        numero = numero.replace("d", "");

        numero = numero.substr(0, numero.length - 4);

        if (user != null) {
            // Log de usuário
            var serv = baseURL + '/api/Log/salvarLogUsuario';
            var met = 'POST';
            var head = { 'Content-Type': 'application/json' };
            var desc = 'O atendente realizou a consulta pela denúncia D' + numero + ano;
            var data = { idUsuario: user.ID_USUARIO, idUsuarioAlterado: 0, descricao: desc };

            $http({ method: met, url: serv, headers: head, params: data })
            .then(
                function successCallback(response) {

                }
                , function errorCallback(response) {
                    ajaxError(response.data);
                });
        }
        else {
            ajaxError('Não foi possível consultar a denúncia.');
        }

        $cookies.put('anoConsultaAnalise', ano, { path: '/' });
        $cookies.put('numConsultaAnalise', numero, { path: '/' });
        $window.location.href = baseURL + '/Consulta/ConsultarDenuncia';
    }

    ///* Avançada */

    //$scope.carregado = null;
    //$scope.denuncias = null;
    //$scope.situacoes = null;
    //$scope.crimes = null;
    //$scope.delegacias = null;
    //$scope.batalhoes = null;
    //$scope.unidades = null;
    //$scope.pesquisa = {};

    //$scope.carregarPagina = function () {
    //    $scope.carregarSituacao();
    //    $scope.carregarTipoCrimes();
    //    $scope.carregarDelegacias();
    //    $scope.carregarBatalhoes();
    //}

    //$scope.carregarSituacao = function () {
    //    var serv = baseURL + '/api/Denuncia/listarSituacao';
    //    var met = 'GET';
    //    var head = { 'Content-Type': 'application/json' };
    //    $scope.crimes = null;

    //    $http({ method: met, url: serv, headers: head, params: data })
    //    .then(
    //        function successCallback(response) {
    //            $scope.situacoes = response.data;
    //        }
    //        , function errorCallback(response) {
    //            ajaxError(response.data);
    //        });
    //}

    //$scope.carregarDelegacias = function () {
    //    var serv = baseURL + '/api/Denuncia/listarDelegacias';
    //    var met = 'GET';
    //    var head = { 'Content-Type': 'application/json' };
    //    $scope.delegacias = null;

    //    $http({ method: met, url: serv, headers: head, params: data })
    //    .then(
    //        function successCallback(response) {
    //            $scope.delegacias = response.data;
    //        }
    //        , function errorCallback(response) {
    //            ajaxError(response.data);
    //        });
    //}

    //$scope.carregarBatalhoes = function () {
    //    var serv = baseURL + '/api/Denuncia/listarBatalhoes';
    //    var met = 'GET';
    //    var head = { 'Content-Type': 'application/json' };
    //    $scope.batalhoes = null;

    //    $http({ method: met, url: serv, headers: head, params: data })
    //    .then(
    //        function successCallback(response) {
    //            $scope.batalhoes = response.data;
    //        }
    //        , function errorCallback(response) {
    //            ajaxError(response.data);
    //        });
    //}

    //$scope.carregarTipoCrimes = function () {
    //    var serv = baseURL + '/api/Denuncia/listarTipoCrime';
    //    var met = 'GET';
    //    var head = { 'Content-Type': 'application/json' };
    //    $scope.crimes = null;

    //    $http({ method: met, url: serv, headers: head, params: data })
    //    .then(
    //        function successCallback(response) {
    //            $scope.crimes = response.data;
    //        }
    //        , function errorCallback(response) {
    //            ajaxError(response.data);
    //        });
    //}

    //$scope.selecionaUnidade = function () {
    //    switch ($scope.pesquisa.ID_TIPO_ORGAO)
    //    {
    //        default:
    //            $scope.unidades = null;
    //            break;
    //        case 1:
    //            $scope.unidades = $scope.delegacias;
    //            break;
    //        case 2:
    //            $scope.unidades = $scope.batalhoes;
    //            break;
    //    }
    //}

    $scope.limparDados = function () {
        $scope.ConsultaViewModel = {
            DataEncaminhamentoInicial: null,
            DataEncaminhamentoFinal: null,
            DataCadastroInicial: null,
            DataCadastroFinal: null,
            SituacaoId: "",
            UnidadeId: "",
            TipoCrimeId: "",
            CpfDenunciante: "",
            DescricaoTipoCrime: "",
        };

        $scope.finalizada = "1";
        $scope.expirar = "";
        $scope.gridOptions.data = "";
    }

    $scope.ValidaParametrosPesquisa = function () {
        var isValid = true;

        if ($scope.ConsultaViewModelForm.$invalid) {
            isValid = false;

            ajaxError("Informe os filtros obrigatórios para pesquisa.");

            angular.forEach($scope.ConsultaViewModelForm.$error, function (field) {
                angular.forEach(field, function (errorField) {
                    if (errorField.$setTouched) {
                        errorField.$setTouched();
                    }
                })
            });
        }

        return isValid;
    };

    // Monta os filtro enviados na Pesquisa
    $scope.BuildODataFilter = function () {
        if (!$scope.ValidaParametrosPesquisa())
            return;

        $scope.ODataFilter = "&orgao='" + $scope.ConsultaViewModel.OrgaoIdTipo + "'";

        var first = true;

        if (angular.isDefined($scope.ConsultaViewModel.TipoCrimeId) && $scope.ConsultaViewModel.TipoCrimeId != 0 && $scope.ConsultaViewModel.TipoCrimeId != "") {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'tipo_crime/ID_TIPO_CRIME eq ' + $scope.ConsultaViewModel.TipoCrimeId;
        }

        if (angular.isDefined($scope.ConsultaViewModel.DescricaoTipoCrime) && $scope.ConsultaViewModel.DescricaoTipoCrime != "") {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + ' startswith(outro_tipo_crime, ' + "'" + $scope.ConsultaViewModel.DescricaoTipoCrime.toString() + "'" + ')  ';
        }

        if (angular.isDefined($scope.ConsultaViewModel.SituacaoId) && $scope.ConsultaViewModel.SituacaoId != 0 && $scope.ConsultaViewModel.SituacaoId != "") {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'situacao/ID_SITUACAO eq ' + $scope.ConsultaViewModel.SituacaoId;
        }

        if ($scope.finalizada == 2) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'finalizada eq ' + false;
        }

        if ($scope.finalizada == 3) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'finalizada eq ' + true; 
        }


        if (angular.isDate($scope.ConsultaViewModel.DataCadastroInicial)) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ConsultaViewModel.DataCadastroInicial.setHours(0);
            $scope.ConsultaViewModel.DataCadastroInicial.setMinutes(0);
            $scope.ConsultaViewModel.DataCadastroInicial.setSeconds(0);

            $scope.ODataFilter = $scope.ODataFilter + "data_inclusao ge " + $scope.ConsultaViewModel.DataCadastroInicial.toISOString();
        }

        if (angular.isDate($scope.ConsultaViewModel.DataCadastroFinal)) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ConsultaViewModel.DataCadastroFinal.setHours(23);
            $scope.ConsultaViewModel.DataCadastroFinal.setMinutes(59);
            $scope.ConsultaViewModel.DataCadastroFinal.setSeconds(59);

            $scope.ODataFilter = $scope.ODataFilter + "data_inclusao le " + $scope.ConsultaViewModel.DataCadastroFinal.toISOString();
        }

        if (angular.isDate($scope.ConsultaViewModel.DataEncaminhamentoInicial)) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ConsultaViewModel.DataEncaminhamentoInicial.setHours(0);
            $scope.ConsultaViewModel.DataEncaminhamentoInicial.setMinutes(0);
            $scope.ConsultaViewModel.DataEncaminhamentoInicial.setSeconds(0);

            $scope.ODataFilter = $scope.ODataFilter + "data_encaminhamento ge " + $scope.ConsultaViewModel.DataEncaminhamentoInicial.toISOString();
        }

        if (angular.isDate($scope.ConsultaViewModel.DataEncaminhamentoFinal)) {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ConsultaViewModel.DataEncaminhamentoFinal.setHours(23);
            $scope.ConsultaViewModel.DataEncaminhamentoFinal.setMinutes(59);
            $scope.ConsultaViewModel.DataEncaminhamentoFinal.setSeconds(59);

            $scope.ODataFilter = $scope.ODataFilter + "data_encaminhamento le " + $scope.ConsultaViewModel.DataEncaminhamentoFinal.toISOString();
        }

        if ($scope.isCPFValid($scope.ConsultaViewModel.CpfDenunciante) && $scope.ConsultaViewModel.CpfDenunciante != "") {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'cpf_denunciante eq ' + "'" + $scope.ConsultaViewModel.CpfDenunciante.toString() + "'";
        }

        if (angular.isDefined($scope.ConsultaViewModel.UnidadeId) && $scope.ConsultaViewModel.UnidadeId != 0 && $scope.ConsultaViewModel.UnidadeId != "") {
            if (first) {
                $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                first = false;
            }
            else {
                $scope.ODataFilter = $scope.ODataFilter + " and ";
            }

            $scope.ODataFilter = $scope.ODataFilter + 'orgao/ID_ORGAO eq ' + $scope.ConsultaViewModel.UnidadeId;
        }

        // Regra para o filtro Expira em
        // Recupera a data atual
        var dataAtualSemAlteracao = new Date();
        var dataAtual = new Date();

        if ($scope.expirar == 11) {

            if (angular.isDate(dataAtual)) {
                if (first) {
                    $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                    first = false;
                }
                else {
                    $scope.ODataFilter = $scope.ODataFilter + " and ";
                }

                dataAtual.setHours(23);
                dataAtual.setMinutes(59);
                dataAtual.setSeconds(59);

                dataAtualSemAlteracao.setHours(23)
                dataAtualSemAlteracao.setMinutes(59);
                dataAtualSemAlteracao.setSeconds(59);

                $scope.ODataFilter = $scope.ODataFilter + "data_expira le " + dataAtualSemAlteracao.toISOString();
            }
        } else if ($scope.expirar == 10) {
            // Recupera os itens expirados com até 10 dias 
            var dataAtual = new Date();
            dataAtual.setDate(dataAtual.getDate() + 10);

            if (angular.isDate(dataAtual)) {
                if (first) {
                    $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                    first = false;
                }
                else {
                    $scope.ODataFilter = $scope.ODataFilter + " and ";
                }

                dataAtualSemAlteracao.setHours(23)
                dataAtualSemAlteracao.setMinutes(59);
                dataAtualSemAlteracao.setSeconds(59);

                $scope.ODataFilter = $scope.ODataFilter + "data_expira ge " + dataAtualSemAlteracao.toISOString();

                $scope.ODataFilter = $scope.ODataFilter + " and data_expira le " + dataAtual.toISOString();
            }
        } else if ($scope.expirar == 5) {
            // Recupera os itens expirados com até 5 dias 
            dataAtual.setDate(dataAtual.getDate() + 5);

            if (angular.isDate(dataAtual)) {
                if (first) {
                    $scope.ODataFilter = $scope.ODataFilter + '&$filter=';
                    first = false;
                }
                else {
                    $scope.ODataFilter = $scope.ODataFilter + " and ";
                }

                dataAtualSemAlteracao.setHours(23)
                dataAtualSemAlteracao.setMinutes(59);
                dataAtualSemAlteracao.setSeconds(59);

                $scope.ODataFilter = $scope.ODataFilter + "data_expira ge " + dataAtualSemAlteracao.toISOString();

                $scope.ODataFilter = $scope.ODataFilter + " and data_expira le " + dataAtual.toISOString();
            }
        }
    };

    $scope.pesquisarDenuncia = function () {
        $scope.BuildODataFilter();       

        if (!$scope.ValidaParametrosPesquisa())
            return;

        //Carrega os dados do Grid.
        $scope.Search();

        //Carrega os dados para exportação do Grid.
        $scope.SearchExportacao();
    }
        //var user = $cookies.getObject('retorno');

        //if (user != null) {
        //    var serv = baseURL + '/api/Consulta/pesquisaAvancada';
        //    var met = 'GET';
        //    var head = { 'Content-Type': 'application/json' };
        //    var data = {  }
        //    $scope.denuncias = null;

        //    $http({ method: met, url: serv, headers: head, params: data })
        //    .then(
        //        function successCallback(response) {
        //        }
        //        , function errorCallback(response) {
        //            ajaxError(response.data);
        //        });
        //}
        //else {
        //    ajaxError('Não foi possível abrir a denúncia.');
        //}
    
    //$scope.carregarPagina();

    /********/

    // Validação de CPF
    $scope.isCPFValid = function (CPFValue) {
        

        if (CPFValue != undefined && CPFValue != "") {
            var cpf = CPFValue.replace(/[^\d]+/g, '');
            if (cpf === '') return false;
            // Elimina CPFs invalidos conhecidos    
            if (cpf.length !== 11 ||
                cpf === "00000000000" ||
                cpf === "11111111111" ||
                cpf === "22222222222" ||
                cpf === "33333333333" ||
                cpf === "44444444444" ||
                cpf === "55555555555" ||
                cpf === "66666666666" ||
                cpf === "77777777777" ||
                cpf === "88888888888" ||
                cpf === "99999999999")
                return false;
            // Valida 1o digito 
            var add = 0;
            for (i = 0; i < 9; i++)
                add += parseInt(cpf.charAt(i)) * (10 - i);
            var rev = 11 - (add % 11);
            if (rev === 10 || rev === 11)
                rev = 0;
            if (rev !== parseInt(cpf.charAt(9)))
                return false;
            // Valida 2o digito 
            add = 0;
            for (var i = 0; i < 10; i++)
                add += parseInt(cpf.charAt(i)) * (11 - i);
            rev = 11 - (add % 11);
            if (rev === 10 || rev === 11)
                rev = 0;
            if (rev !== parseInt(cpf.charAt(10)))
                return false;

            return true;
        }
        else
        {
            return true;
        }
    };

    $scope.exportaPDF = function () {
        var conteudo = [];

        // Cabeçalho
        conteudo.push({ image: 'brasaoSP', width: 70, height: 77, absolutePosition: { x: 20, y: 25 } });
        conteudo.push({ text: 'GOVERNO DO ESTADO DE SÃO PAULO', style: 'gov_header' });
        conteudo.push({ text: 'SECRETARIA DE ESTADO DA SEGURANÇA PÚBLICA', style: 'ssp_header' });
        conteudo.push({ text: 'DEPA - Delegacia Eletrônica de Proteção Animal', style: 'depa_header' });
        conteudo.push({ text: ' ', style: 'den_space1' });

        // Conteúdo
        // Montando a tabela de acordo com os valores do grid
        var tabela = [];
        // Cabeçalho da tabela
        tabela.push([
            { text: 'Protocolo', style: 'table_header' },
            { text: 'Data de Cadastro', style: 'table_header' },
            { text: 'CPF do Denunciante', style: 'table_header' },
            { text: 'Situação', style: 'table_header' },
            { text: 'Tipo de Crime', style: 'table_header' },            
            { text: 'Data de Encaminhamento', style: 'table_header' },
            { text: 'Unidade Policial', style: 'table_header' },
            { text: 'A expirar', style: 'table_header' }
        ]);

        //for (var i = 0; i < $scope.gridExportApi.grid.rows.length; i++) {
        //    var protocolo = 'D' + $scope.gridExportApi.grid.rows[i].entity.numero_denuncia + $scope.gridExportApi.grid.rows[i].entity.ano_denuncia;
        //    var data_inclusao = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_inclusao, 'dd/MM/yyyy HH:mm:ss');
        //    var data_encaminhamento = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_encaminhamento, 'dd/MM/yyyy');
        //    var data_expira = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_expira, 'dd/MM/yyyy');
        //    var cpf_denunciante = $scope.gridExportApi.grid.rows[i].entity.cpf_denunciante;
        //    var situacao = $scope.gridExportApi.grid.rows[i].entity.situacao.DESCRICAO_SITUACAO;
        //    var tipo_crime = $scope.gridExportApi.grid.rows[i].entity.tipo_crime.DESCRICAO_TIPO_CRIME;
        //    var unid_policial = $scope.gridExportApi.grid.rows[i].entity.orgao.NOME_ORGAO;

        //    tabela.push([protocolo, data_inclusao, cpf_denunciante, situacao, tipo_crime, unid_policial, data_encaminhamento, data_expira]);
        //}


        for (var i = 0; i < $scope.gridExportApi.grid.rows.length; i++) {
            var protocolo = 'D' + $scope.gridExportApi.grid.rows[i].entity.numero_denuncia + $scope.gridExportApi.grid.rows[i].entity.ano_denuncia;
            var data_inclusao = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_inclusao, 'dd/MM/yyyy HH:mm:ss');

            var data_encaminhamento = '';
            if ($scope.gridExportApi.grid.rows[i].entity.data_encaminhamento != null)
                data_encaminhamento = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_encaminhamento, 'dd/MM/yyyy')

            var data_expira = '';
            if ($scope.gridExportApi.grid.rows[i].entity.data_expira != null)
                data_expira = $filter('date')($scope.gridExportApi.grid.rows[i].entity.data_expira, 'dd/MM/yyyy');

            var cpf_denunciante = $scope.gridExportApi.grid.rows[i].entity.cpf_denunciante;
            var situacao = $scope.gridExportApi.grid.rows[i].entity.situacao.DESCRICAO_SITUACAO;
            var tipo_crime = $scope.gridExportApi.grid.rows[i].entity.tipo_crime.DESCRICAO_TIPO_CRIME;

            var unid_policial = "";

            if (angular.isDefined($scope.gridExportApi.grid.rows[i].entity.orgao) && $scope.gridExportApi.grid.rows[i].entity.orgao != null)
            {
                unid_policial = $scope.gridExportApi.grid.rows[i].entity.orgao.NOME_ORGAO;
            }

            tabela.push([protocolo, data_inclusao, cpf_denunciante, situacao, tipo_crime, data_encaminhamento, unid_policial, data_expira]);
        }

        conteudo.push({ table: { body: tabela } });

        // Gerando as informações do PDF
        $scope.exportDen = {
            content: conteudo,
            images: { brasaoSP: 'data:image/png;base64,' },
            styles: {
                gov_header: { bold: true, alignment: 'center', fontSize: 18 },
                ssp_header: { alignment: 'center', fontSize: 12 },
                table_header: { color: 'steelblue' },
                depa_header: { bold: true, alignment: 'center', fontSize: 14 },
                den_header: { bold: true, fontSize: 18 },
                den_titleleft: { bold: true, fontSize: 18, alignment: 'left' },
                den_titleright: { bold: true, fontSize: 18, alignment: 'right' },
                den_protocolo: { bold: true, fontSize: 14 },
                den_title: { bold: true, fontSize: 18 },
                den_subtitle: { bold: true, fontSize: 16, color: '#366696' },
                den_sigilo: { bold: true, fontSize: 14, color: '#FF3333', alignment: 'center' },
                den_space1: { fontSize: 20 },
                den_space2: { fontSize: 6 },
                den_space3: { fontSize: 10 },
                just_text: { alignment: 'justify' }
            },
            pageSize: 'A4',
            pageOrientation: 'landscape'
        };

        // Enviando o download do PDF ao cliente
        pdfMake.createPdf($scope.exportDen).download('ConsultaDenuncia.pdf');
    }
}]);