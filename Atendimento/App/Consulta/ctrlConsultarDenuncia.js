SSPApp.controller('ctrlConsultarDenuncia', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL) {

    $scope.denuncia = null;
    $scope.vazio = null;
    $scope.resposta = '';
    $scope.tipos = null;
    $scope.tipo_resposta = null;

    // Carrega as informações de acordo com os dados passados
    $scope.carregar = function () {
        
        var numero = $cookies.get('numConsultaAnalise');
        var ano = $cookies.get('anoConsultaAnalise');
    ...
    $scope.carregarDenuncia = function (ano, num) {
        ...
        if (user != null) {
            var serv = baseURL + '/api/Atendimento/abrirDenunciaCompleto';
            var met = 'GET';
            ...
            .then(
                function successCallback(response) {
                    ...
                    if ($scope.vazio) {
                        desc = 'Não foi encontrado a denúncia D' + num + ano;
                        $scope.logUsuario(desc);
                    }
                    else {
                        desc = "O atendente '" + user.NOME_COMPLETO + "' consultou a denúncia D" + num + ano;
                        $scope.logDenuncia(ano, num, desc);
                    }
                ...
                , function errorCallback(response) {
                    ...
                    if (response.data.Message == "The request is invalid.") {
                        ajaxError("O valor digitado é inválido!");
                    }
                    else {
    ...
    $scope.carregarTipos = function () {
        var serv = baseURL + '/api/Apuracao/listarTipoBoletim';
        var met = 'GET';
        var head = { 'Content-Type': 'application/json' };
        $scope.tipos = null;
    ...
    $scope.logUsuario = function (desc) {
        var user = $cookies.getObject('retorno');
    ...
    $scope.logDenuncia = function (ano, num, desc) {
        var user = $cookies.getObject('retorno');

        if (user != null) {
            var serv = baseURL + '/api/Log/salvarLogDenuncia';
            var met = 'POST';
            var head = { 'Content-Type': 'application/json' };
            var data = { ano: ano, numero: num, idUsuario: user.ID_USUARIO, descricao: desc };
    ...
    $scope.responderDenuncia = function () {
        var user = $cookies.getObject('retorno');

        if (user != null) {

            if ($scope.tipo_resposta == null) {
                ajaxError('Marque a finalização de resultado positivo ou negativo da denúncia.');
            }
            else {
                if ($scope.tipo_resposta && ($scope.data_providencia == null || $scope.data_providencia.trim() == '')) {
                    ajaxError('Para que a denúncia seja respondida, é necessário preencher a data em que a providência foi tomada!');
                }
                else if ($scope.resposta == null || $scope.resposta.trim() == '') {
                    ajaxError('Para que a denúncia seja respondida, é necessário preencher a resposta!');
                }
                else {
                    var serv = baseURL + '/api/Apuracao/responderDenuncia';
                    var met = 'POST';
                    var head = { 'Content-Type': 'application/json' };
                    var data = {
                        ano: $scope.ano,
                        numero: $scope.numero,
                        idAtend: user.ID_USUARIO,
                        just: $scope.resposta,
                        idTipoBoletim: $scope.tipo_resposta ? '' : $scope.tipo_boletim,
                        NumBO: $scope.tipo_resposta ? '' : $scope.num_boletim,
                        dataProvidencia: $scope.tipo_resposta ? '' : $scope.data_providencia,
                        tipoResposta: $scope.tipo_resposta,
                        civil: user.FLAG_CIVIL,
                        militar: user.FLAG_MILITAR
                    };
    ...
    $scope.marcarTipo = function (value) {
        $scope.tipo_resposta = value;
    }

    // Retorna o usuário para a lista de análise
    $scope.retornar = function () {
        $cookies.remove('anoConsultaAnalise', { path: '/' });
        $cookies.remove('numConsultaAnalise', { path: '/' });
        $window.location.href = baseURL + '/Consulta/Consultar';
    }
    ...
    $scope.exportar = function () {
        if ($scope.denuncia != null) {
            ...
            conteudo.push({ text: ' ', style: 'den_space1' });
            conteudo.push({ text: [{ text: 'Situação Polícia Civil: ', bold: true }, { text: $scope.denuncia.status_pc }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Situação Polícia Militar: ', bold: true }, { text: $scope.denuncia.status_pm }] });
            conteudo.push({ text: ' ', style: 'den_space1' });
            ...
            conteudo.push({ text: 'Polícia Civil', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });

            if ($scope.denuncia.id_status_pc == 1 || $scope.denuncia.id_status_pc == 2 || $scope.denuncia.id_status_pc == 9) {
                conteudo.push({ text: 'Aguardando a análise da denúncia.', bold: true });
            }

            if ($scope.denuncia.id_status_pc == 3 || $scope.denuncia.id_status_pc == 4) {
                conteudo.push({ text: 'A análise da denúncia resultou no descarte dela.', bold: true });
            }

            if ($scope.denuncia.id_status_pc >= 5 && $scope.denuncia.id_status_pc != 9) {
                conteudo.push({ text: [{ text: 'Encaminhado para a unidade policial: ', bold: true }, { text: $scope.denuncia.orgao_pc }] });
                conteudo.push({ text: ' ', style: 'den_space2' });

                if ($scope.denuncia.just_reativo_pc != null && $scope.denuncia.just_reativo_pc.trim() != '')
                    conteudo.push({ text: [{ text: 'Justificativa de reativação: ', bold: true }, { text: $scope.denuncia.just_reativo_pc }] });
                else
                    conteudo.push({ text: 'Não houve reativação nesta denúncia.', bold: true });
            }

            conteudo.push({ text: ' ', style: 'den_space3' });

            conteudo.push({ text: 'Polícia Militar', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });

            if ($scope.denuncia.id_status_pm == 1 || $scope.denuncia.id_status_pm == 2 || $scope.denuncia.id_status_pm == 9) {
                conteudo.push({ text: 'Aguardando a análise da denúncia.', bold: true });
            }

            if ($scope.denuncia.id_status_pm == 3 || $scope.denuncia.id_status_pm == 4) {
                conteudo.push({ text: 'A análise da denúncia resultou no descarte dela.', bold: true });
            }

            if ($scope.denuncia.id_status_pm >= 5 && $scope.denuncia.id_status_pm != 9) {
                conteudo.push({ text: [{ text: 'Encaminhado para a unidade policial: ', bold: true }, { text: $scope.denuncia.orgao_pm }] });
                conteudo.push({ text: ' ', style: 'den_space2' });

                if ($scope.denuncia.just_reativo_pm != null && $scope.denuncia.just_reativo_pm.trim() != '')
                    conteudo.push({ text: [{ text: 'Justificativa de reativação: ', bold: true }, { text: $scope.denuncia.just_reativo_pm }] });
                else
                    conteudo.push({ text: 'Não houve reativação nesta denúncia.', bold: true });
            }
            ...
            // Apuração/Resposta
            conteudo.push({ text: 'Apuração/Resposta', style: 'den_title' });
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: 'Polícia Civil', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });
            ...
            if ($scope.denuncia.id_status_pc >= 5 && $scope.denuncia.id_status_pc != 9) {
                conteudo.push({ text: [{ text: 'Unidade policial que recebeu a denúncia: ', bold: true }, { text: $scope.denuncia.orgao_pc }] });

                if ($scope.denuncia.id_status_pc > 5) {
                    if ($scope.denuncia.just_retorno_pc != null && $scope.denuncia.just_retorno_pc.trim() != '') 
                        conteudo.push({ text: [{ text: 'Justificativa do retorno: ', bold: true }, { text: $scope.denuncia.just_retorno_pc }] });
                    else
                        conteudo.push({ text: 'Não houve retorno de resposta insuficiente pela administração.', bold: true });

                    conteudo.push({ text: [{ text: 'A apuração da denúncia foi finalizada com resultado positivo? ', bold: true }, { text: $scope.denuncia.flag_resposta_pc }] });

                    if ($scope.denuncia.flag_resposta_pc != null && $scope.denuncia.flag_resposta_pc == 'Sim') {
                        conteudo.push({ text: [{ text: 'Número de boletim: ', bold: true }, { text: $scope.denuncia.numero_boletim_pc }] });
                        conteudo.push({ text: [{ text: 'Tipo de boletim: ', bold: true }, { text: $scope.denuncia.tipo_boletim_pc }] });
                        conteudo.push({ text: [{ text: 'Data de providência: ', bold: true }, { text: $scope.denuncia.data_providencia_pc }] });
                    }

                    conteudo.push({ text: [{ text: 'Resposta enviada pela unidade policial: ', bold: true }, { text: $scope.denuncia.resposta_pc }] });
                }
            }

            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: 'Histórico', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });

            angular.forEach($scope.denuncia.historico_pc, function (hist, key) {
                var indHist = key + 1;
                conteudo.push({ text: 'Histórico ' + indHist, bold: true });
                conteudo.push({ text: ' ', style: 'den_space2' });
                conteudo.push({ text: [{ text: 'Resposta enviada pela unidade policial: ', bold: true }, { text: hist.resposta }] });
                conteudo.push({ text: ' ', style: 'den_space2' });
                conteudo.push({ text: [{ text: 'Data em que a resposta foi enviada: ', bold: true }, { text: hist.data_resposta }] });

                if (hist.just_retorno != null) {
                    conteudo.push({ text: ' ', style: 'den_space2' });
                    conteudo.push({ text: [{ text: 'Justificativa do retorno: ', bold: true }, { text: hist.just_retorno }] });
                    conteudo.push({ text: ' ', style: 'den_space2' });
                    conteudo.push({ text: [{ text: 'Data do retorno da resposta: ', bold: true }, { text: hist.data_retorno }] });
                }
            ...
            if ($scope.denuncia.id_status_pm >= 5 && $scope.denuncia.id_status_pm != 9) {
                conteudo.push({ text: [{ text: 'Unidade policial que recebeu a denúncia: ', bold: true }, { text: $scope.denuncia.orgao_pm }] });

                if ($scope.denuncia.id_status_pm > 5) {
                    if ($scope.denuncia.just_retorno_pm != null && $scope.denuncia.just_retorno_pm.trim() != '')
                        conteudo.push({ text: [{ text: 'Justificativa do retorno: ', bold: true }, { text: $scope.denuncia.just_retorno_pm }] });
                    else
                        conteudo.push({ text: 'Não houve retorno de resposta insuficiente pela administração.', bold: true });

                    conteudo.push({ text: [{ text: 'A apuração da denúncia foi finalizada com resultado positivo? ', bold: true }, { text: $scope.denuncia.flag_resposta_pm }] });

                    if ($scope.denuncia.flag_resposta_pm != null && $scope.denuncia.flag_resposta_pm == 'Sim') {
                        conteudo.push({ text: [{ text: 'Número de boletim: ', bold: true }, { text: $scope.denuncia.numero_boletim_pm }] });
                        conteudo.push({ text: [{ text: 'Tipo de boletim: ', bold: true }, { text: $scope.denuncia.tipo_boletim_pm }] });
                        conteudo.push({ text: [{ text: 'Data de providência: ', bold: true }, { text: $scope.denuncia.data_providencia_pm }] });
                    }

                    conteudo.push({ text: [{ text: 'Resposta enviada pela unidade policial: ', bold: true }, { text: $scope.denuncia.resposta_pm }] });
                }
            }
            ...
            angular.forEach($scope.denuncia.historico_pm, function (hist, key) {
                var indHist = key + 1;
                conteudo.push({ text: 'Histórico ' + indHist, bold: true });
                conteudo.push({ text: ' ', style: 'den_space2' });
                conteudo.push({ text: [{ text: 'Resposta enviada pela unidade policial: ', bold: true }, { text: hist.resposta }] });
            ...
            // Descarte
            conteudo.push({ text: 'Descarte', style: 'den_title' });
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: 'Polícia Civil', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });
            ...
            if ($scope.denuncia.id_status_pc == 3) {
                conteudo.push({ text: 'A análise da denúncia resultou no descarte dela.', bold: true });
            }

            if ($scope.denuncia.id_status_pc == 4) {
                conteudo.push({ text: [{ text: 'Motivo do descarte: ', bold: true }, { text: $scope.denuncia.just_descarte_pc }] });
            }

            if ($scope.denuncia.id_status_pc >= 5 && $scope.denuncia.id_status_pc != 9) {
                conteudo.push({ text: 'Foi realizado o encaminhamento da denúncia.', bold: true });                
            }
            ...
            if ($scope.denuncia.id_status_pm == 3) {
                conteudo.push({ text: 'A análise da denúncia resultou no descarte dela.', bold: true });
            }

            if ($scope.denuncia.id_status_pm == 4) {
                conteudo.push({ text: [{ text: 'Motivo do descarte: ', bold: true }, { text: $scope.denuncia.just_descarte_pm }] });
            }
            ...
            // Validação
            conteudo.push({ text: 'Validação', style: 'den_title' });
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: 'Polícia Civil', style: 'den_subtitle' });
            conteudo.push({ text: ' ', style: 'den_space2' });
            ...
            if ($scope.denuncia.id_status_pc >= 5 && $scope.denuncia.id_status_pc <= 7) {
                conteudo.push({ text: 'Aguardando a finalização da denúncia.', bold: true });
            }

            if ($scope.denuncia.id_status_pc == 8) {
                conteudo.push({ text: [{ text: 'Resposta encaminhada para o cidadão: ', bold: true }, { text: $scope.denuncia.resposta_final_pc }] });
            }
            ...
            if ($scope.denuncia.id_status_pm >= 5 && $scope.denuncia.id_status_pm <= 7) {
                conteudo.push({ text: 'Aguardando a finalização da denúncia.', bold: true });
            }

            if ($scope.denuncia.id_status_pm == 8) {
                conteudo.push({ text: [{ text: 'Resposta encaminhada para o cidadão: ', bold: true }, { text: $scope.denuncia.resposta_final_pm }] });
            }
            ...
            pdfMake.createPdf($scope.exportDen).download('Consulta - ' + protocolo + '.pdf');
        }
    }
