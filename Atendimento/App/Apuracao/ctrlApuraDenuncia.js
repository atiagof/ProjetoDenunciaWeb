SSPApp.controller('ctrlApuraDenuncia', ['$scope', '$http', '$controller', '$uibModal', '$timeout', '$window', 'uiGridConstants', '$cookies', 'baseURL', function ($scope, $http, $controller, $uibModal, $timeout, $window, uiGridConstants, $cookies, baseURL) {

    $scope.denuncia = null;
    $scope.vazio = null;
    $scope.resposta = '';
    $scope.lim_resposta = 1000;
    $scope.lim_numeroBO = 12;
    $scope.tipos = null;
    $scope.delegacias = null;
    $scope.tipo_resposta = null;
    $scope.registro_bo = null;
    $scope.num_boletim = '';
    $scope.del_elaboracao = 0;
    $scope.tipo_boletim = 0;
    $scope.today = new Date();

    // Carrega as informações de acordo com os dados passados
    $scope.carregar = function () {
        var ano = $cookies.get('anoApurar');
        var numero = $cookies.get('numApurar');
    ...
    $scope.carregarDenuncia = function (ano, num) {
        ...
        if (user != null) {
            var serv = baseURL + '/api/Apuracao/abrirDenuncia';
            var met = 'GET';
    ...
    // Carrega a lista com os tipos de boletins
    ...
    // Carrega a lista de delegacias
    $scope.carregarDelegacias = function () {
        var serv = baseURL + '/api/Apuracao/listarDelegacias';
        var met = 'GET';
        var head = { 'Content-Type': 'application/json' };
        $scope.tipos = null;
    ...
    $scope.checarResposta = function () {
        if ($scope.resposta.length > $scope.lim_resposta)
            $scope.resposta = $scope.resposta.slice(0, $scope.lim_resposta);
    }

    // Função para checar o limite do preenchimento do número de boletim
    $scope.checarNumBO = function () {
        if ($scope.num_boletim.length > $scope.lim_numeroBO)
            $scope.num_boletim = $scope.num_boletim.slice(0, $scope.lim_numeroBO);
    }
    ...
    $scope.verificarDenuncia = function () {
        ...
        if (user != null) {
            if ($scope.denuncia.usuario_resposta.toUpperCase() != user.NOME.toUpperCase())
                return true;
            else
                return false;
        }
    ...
    $scope.verificarResposta = function () {
        if ($scope.verificarDenuncia()) {
            $('#descResposta').modal('hide');
            $('#checkResp').modal({});
        } else {
            $scope.responderDenuncia();
        }
    }

    // Função para retornar o indice do tipo de boletim pelo nome
    $scope.returnTipoIndex = function (TipoBoletim) {
        var result = '';

        for (var i = 0; $scope.tipos != null && i < $scope.tipos.length; i++) {
            if ($scope.tipos[i].tipoBoletim == TipoBoletim) {
                result = $scope.tipos[i].idTipoBoletim;
                break;
            }
        }
    ...
    // Função para retornar o indice da delegacia pelo nome
    $scope.returnDelegaciaIndex = function (delegacia) {
        var result = '';

        for (var i = 0; $scope.delegacias != null && i < $scope.delegacias.length; i++) {
            if ($scope.delegacias[i].NOME_ORGAO == delegacia) {
                result = $scope.delegacias[i].ID_ORGAO;
                break;
            }
        }
    ...
    $scope.responderDenuncia = function () {
        ...
        if (user != null) {
            ...
            if ($scope.denuncia.status == "Resposta insuficiente") {
                switch ($scope.denuncia.flag_resposta) {
                    default:
                        $scope.tipo_resposta = null;
                        break;
                    case "Não":
                        $scope.tipo_resposta = false;
                        break;
                    case "Sim":
                        $scope.tipo_resposta = true;
                        break;
                }

                switch ($scope.denuncia.flag_registroBO) {
                    default:
                        $scope.registro_bo = null;
                        break;
                    case "Não":
                        $scope.registro_bo = false;
                        break;
                    case "Sim":
                        $scope.registro_bo = true;
                        $scope.data_providencia = $scope.denuncia.data_providencia;
                        $scope.tipo_boletim = $scope.returnTipoIndex($scope.denuncia.tipo_boletim);
                        $scope.del_elaboracao = $scope.returnDelegaciaIndex($scope.denuncia.delegacia_elaboracao);
                        $scope.num_boletim = $scope.denuncia.numero_boletim;
                        break;
                }
            }
            ...
            else if ($scope.registro_bo == null) {
                ajaxError('Responda se houve o registro de um boletim de ocorrência para a denúncia.');
            }
            else
            {
                if ($scope.registro_bo && $scope.data_providencia == null) {
                    ajaxError('Para que a denúncia seja respondida, é necessário preencher a data em que a providência foi tomada!');
                }
                else if ($scope.registro_bo & $scope.data_providencia > $scope.today) {
                    ajaxError('A data de providência informada é inválida, ela deve ser menor ou igual a data corrente!')
                }
                else if ($scope.registro_bo && ($scope.del_elaboracao == null || $scope.del_elaboracao <= 0)) {
                    ajaxError('Para informar o boletim de ocorrência registrado, é necessário informar a delegacia de elaboração!');
                }
                else if ($scope.registro_bo &&  $scope.num_boletim != null && $scope.num_boletim.trim() == '') {
                    ajaxError('Para informar o boletim de ocorrência registrado, é necessário informar o número dele!');
                }
                else if ($scope.registro_bo && ($scope.tipo_boletim == null || $scope.tipo_boletim <= 0)) {
                    ajaxError('Para informar o boletim de ocorrência registrado, é necessário informar qual é o tipo dela!');
                }
                    ...
                    var data = {
                        ...
                        just: $scope.resposta,
                        idDelegacia: $scope.registro_bo ? $scope.del_elaboracao : 0,
                        idTipoBoletim: $scope.registro_bo ? $scope.tipo_boletim : 0,
                        delElaboracao: $scope.registro_bo ? $scope.del_elaboracao : 0,
                        NumBO: $scope.registro_bo ? $scope.num_boletim : '',
                        dataProvidencia: $scope.registro_bo ? $scope.data_providencia : '',
                        tipoResposta: $scope.tipo_resposta,
                        registroBO: $scope.registro_bo,
    ...
    $scope.marcarTipo = function (value)
    {
        ...
        if (value) {
            $scope.registro_bo = null;
            $("button[class*='btnResp']").removeClass("focus");
            $(".btnRespSim").addClass("focus");
        } else {
            $scope.registro_bo = false;
            $("button[class*='btnResp']").removeClass("focus");
            $(".btnRespNao").addClass("focus");
        }
    }

    $scope.marcarRegistro = function (value) {
        $scope.registro_bo = value;

        if (value) {
            $("button[class*='btnReg']").removeClass("focus");
            $(".btnRegSim").addClass("focus");
        } else {
            $("button[class*='btnReg']").removeClass("focus");
            $(".btnRegNao").addClass("focus");
        }
    }

    // Retorna o usuário para a lista de análise
    $scope.retornar = function () {
        $cookies.remove('anoApurar', { path: '/' });
        $cookies.remove('numApurar', { path: '/' });
        $window.location.href = baseURL + '/Apuracao/Apurar';
    }
    ...
    $scope.exportar = function () {
        if ($scope.denuncia != null) {
            ...
            conteudo.push({ canvas: [{ type: 'line', lineWidth: 2, x1: 0, y1: 5, x2: 516, y2: 5 }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Policial que está respondendo a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_resposta }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Atendente que encaminhou a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_atendimento }] });
            conteudo.push({ text: ' ', style: 'den_space2' });
            conteudo.push({ text: [{ text: 'Unidade policial: ', bold: true }, { text: $scope.denuncia.orgao }] });
            conteudo.push({ text: ' ', style: 'den_space2' });

            if ($scope.denuncia.status == "Resposta insuficiente") {
                conteudo.push({ text: [{ text: 'Atendente que retornou a denúncia: ', bold: true }, { text: $scope.denuncia.usuario_retorno }] });
                conteudo.push({ text: ' ', style: 'den_space2' });
                conteudo.push({ text: [{ text: 'Justificativa do retorno: ', bold: true }, { text: $scope.denuncia.just_retorno }], style: 'just_text' });
                conteudo.push({ text: ' ', style: 'den_space2' });
            }
            ...
            pdfMake.createPdf($scope.exportDen).download('Apuração - ' + protocolo + '.pdf');
        }
    }
